...
with tabs[0]:
    ...
    with col_right:
        ...
        with col_b1:
            bodega_1 = st.selectbox("Bodega 1", lista_bodegas, key="b1")
        with col_e1:
            if bodega_1 != "Sin opciones":
                exp_b1 = bodegas_zona[bodega_1]["Experiencias"]
                exp_nombres_1 = [f"{e['Nombre']} (${e['Precio']:,})" for e in exp_b1]
                idx_exp_1 = st.selectbox("Exp 1", range(len(exp_nombres_1)), format_func=lambda x: exp_nombres_1[x], key="e1")
        ...
        with col_b2:
            bodega_2 = st.selectbox("Bodega 2", lista_bodegas, key="b2")
        with col_e2:
            if bodega_2 != "Sin opciones":
                exp_b2 = bodegas_zona[bodega_2]["Experiencias"]
                exp_nombres_2 = [f"{e['Nombre']} (${e['Precio']:,})" for e in exp_b2]
                idx_exp_2 = st.selectbox("Exp 2", range(len(exp_nombres_2)), format_func=lambda x: exp_nombres_2[x], key="e2")
        ...
        with col_b3:
            bodega_3 = st.selectbox("Bodega 3", lista_bodegas, key="b3")
        with col_e3:
            if bodega_3 != "Sin opciones":
                exp_b3 = bodegas_zona[bodega_3]["Experiencias"]
                exp_nombres_3 = [f"{e['Nombre']} (${e['Precio']:,})" for e in exp_b3]
                idx_exp_3 = st.selectbox("Exp 3", range(len(exp_nombres_3)), format_func=lambda x: exp_nombres_3[x], key="e3")
...
with tabs[3]:
    ...
    if clave_admin == "zelaya123":
        ...
        st.error("Contraseña incorrecta. Intentá nuevamente.")                    tags_lista = [t.strip() for t in nuevos_tags_str.split(",") if t.strip() != ""]
